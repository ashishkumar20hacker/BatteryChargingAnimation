...
import com.batteryanimation.neonbatteryeffects.charge.Activity.SplashActivity;
...
public class LifeCycleOwner implements DefaultLifecycleObserver, Application.ActivityLifecycleCallbacks {
    ...
    public void onStart(@NonNull LifecycleOwner owner) {
        if (Constants.adsResponseModel.getPackage_name() != null && Constants.adsResponseModel.isShow_ads()) {
            if (!isAdShowing && LifeCycleOwner.activity != null && (!LifeCycleOwner.activity.getClass().getName().equals(SplashActivity.class.getName())) && (!LifeCycleOwner.activity.getClass().getName().equals(BatteryChargingAnimationActivity.class.getName()))) {
                isAdShowing = true;
